  ...
  function fc(title, f, type) {
    ...
    return `
      <div class="flight-box" style="margin-bottom:14px;">
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <div style="font-size:11px;font-weight:800;color:var(--gold);letter-spacing:1px;">${title} · ${
            f.date || "未設定日期"
          }</div>
          ${editBtn}
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;">
          <div>
            <div style="font-size:20px;font-weight:900;color:var(--moss);">${
              f.no || "尚未填寫航班號"
            }</div>
...
function openEditHotelModal(index) {
  ...
  const formHtml = `
    <div class="ef-wrap">
      <div class="ef-label">飯店名稱 <span style="color:var(--red);">*</span></div>
      <input type="text" id="editHotelName" class="ef-input" value="${
        h.name || ""
      }">
    </div>
    <div class="ef-wrap">
      <div class="ef-label">飯店地址 (供導航使用)</div>
      <input type="text" id="editHotelAddr" class="ef-input" value="${
        h.addr || ""
      }">
    </div>
    <div style="display:flex;gap:10px;">
      <div class="ef-wrap" style="flex:1;">
        <div class="ef-label">入住日</div>
        <input type="date" id="editHotelCheckin" class="ef-input" value="${
          h.checkin || ""
        }" onchange="autoSyncNights('editHotelCheckin','editHotelCheckout','editHotelNights')">
      </div>
      <div class="ef-wrap" style="flex:1;">
        <div class="ef-label">退房日</div>
        <input type="date" id="editHotelCheckout" class="ef-input" value="${
          h.checkout || ""
        }" onchange="autoSyncNights('editHotelCheckin','editHotelCheckout','editHotelNights')">
      </div>
    </div>
    ...
    <div class="ef-wrap">
      <div class="ef-label">備註說明</div>
      <input type="text" id="editHotelNote" class="ef-input" placeholder="例如: 岡山站直結、附早餐" value="${
        h.note || ""
      }">
    </div>
  `;
